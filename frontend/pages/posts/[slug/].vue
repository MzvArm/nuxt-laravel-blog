  ...
  <div class="post-page">
    <UContainer>
      <UCard class="post-card" radius="3xl" border>
        ...
        <div class="card-head">
          <span class="post-meta">{{ formatDate(post?.published_at || post?.created_at) }} · {{ readingTime }} мин чтения</span>
        </div>

        <h1>{{ post?.title || 'Пост не найден' }}</h1>
        <p>Slug: {{ slug }}</p>
        <p>Post loaded: {{ !!post }}</p>
        ...
        <div class="card-footer">
          <NuxtLink to="/">
            <UButton variant="ghost" size="sm" color="warning" class="u-button">Назад к ленте</UButton>
          </NuxtLink>
        </div>
      </UCard>
    </UContainer>
